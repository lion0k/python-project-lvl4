"""User with headless browser tests."""
import os
import re
...
class TestBrowserHeadless(StaticLiveServerTestCase):
    ...
    def test_index_no_auth(self):
        ...
        self.page.goto(self.get_absolute_url('/'))
        assert self.page.query_selector('text="Статусы"') is None

        self.page.goto(self.get_absolute_url('/statuses/'))
        assert re.search(
    ...
    def test_index(self):
        ...
        assert self.page.query_selector('text="Статусы"') is not None

        self.page.click('text="Статусы"')
        self.page.wait_for_load_state()

        assert self.page.url == self.get_absolute_url('/statuses/')
        row1 = '*css=tr >> text={text}'.format(
            text=self.data['first']['status']['name'],
        )
        row2 = '*css=tr >> text={text}'.format(
            text=self.data['second']['status']['name'],
        )
        row3 = '*css=tr >> text={text}'.format(
            text=self.data['fourth']['status']['name'],
        )
        ...
        assert self.page.query_selector('text="Создать статус"') is not None

    def test_create(self):
        """Test create."""
        data = self.data['first']['user']
        self.login(data)
        self.page.goto(self.get_absolute_url('/statuses/create/'))

        assert self.page.url == self.get_absolute_url('/statuses/create/')
    ...
    def test_update(self):
        ...
        row1 = '*css=tr >> text={text}'.format(text=data['status']['name'])
        row = self.page.query_selector(row1)
        assert row is not None
        row.query_selector('text="Изменить"').click()
        self.page.wait_for_load_state()

        assert re.search(r'/statuses/\d+/update/', self.page.url)
        assert self.page.get_attribute('#id_name', 'value') == data['status']['name']

        data_upd = 'status_update'
    ...
    def test_cannot_update_no_auth(self):
        ...
        self.page.goto(self.get_absolute_url('/statuses/42/update/'))
        self.page.wait_for_load_state()
    ...
    def test_delete(self):
        ...
        status_data = self.data['fourth']['status']
        ...
        assert re.search(r'/statuses/\d+/delete/', self.page.url)
    ...
    def test_cannot_delete_no_auth(self):
        ...
        self.page.goto(self.get_absolute_url('/statuses/42/delete/'))
        self.page.wait_for_load_state()
